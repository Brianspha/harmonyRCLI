...
function getNetworkList() {
  ...
  inquirer.prompt(questions).then((options) => {
    const spinner = ora(
      `Fetching Network Identifiers for shard ${options.shardNo} on the Harmony Blockchain Network ....\n`
    ).start();
    axios({
      method: "POST",
      url: shards[options.shardNo]+"/list",
      data: {
        network_identifier: {
          blockchain: "Harmony",
      ...
      .then((results) => {
        ...
        if (results.data.hasOwnProperty("network_identifiers")) {
          spinner.succeed(
            `Fetching Network Identifiers for shard ${options.shardNo} on the Harmony Blockchain Network ....\n`
          );
          console.log(chalk.yellow("Network Identifiers JSON\n"));
